                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('roles.update', ['role' => $role->id]) }}" method="POST">
                        ...
                        <div>
                            ...
                            <div class="my-3">
                                <input type="text" name="name" value="{{ old('name' , $role->name) }}" class="w-1/2 border-gray-300 rounded-lg shadow-sm" placeholder="Enter Name">
                            </div>
                            @error('name')
                            ...
                            <div class="mb-3 grid grid-cols-4">
                                @if($permissions->isNotEmpty())
                                    @foreach($permissions as $permission)
                                        <div class="mt-3">
                                            <input {{ $hasPermissions->contains($permission->name) ? 'checked' : ''}} type="checkbox" id="permission-{{ $permission->id }}" name="permission[]" value="{{ $permission->name }}" class="rounded">
